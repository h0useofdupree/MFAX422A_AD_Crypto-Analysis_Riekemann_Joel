...
class DataAnalyzer(QMainWindow):
    ...
    def initUI(self):
        ...
        btn_corr = QPushButton("Korrelations-Heatmap", self)
        btn_corr.setStyleSheet(button_style)
        btn_corr.clicked.connect(self.plot_correlation_heatmap)
        button_layout.addWidget(btn_corr)
    ...
    def plot_data(self, file_path, graph_type):
        data = pd.read_csv(
            file_path, parse_dates=["snapped_at"], index_col="snapped_at"
        )
